...
void Game::UpdateModel()
{
	if (gameIsStatted)
	{
		...
		if (!gameOver)
		{
			...
			if (snekMoveCounter >= snekMovePeriod)
			{
				...
				if (!brd.IsInsideBoard(next) || snek.IsInTitleExceptend(next))
				...
				else
				{
					...
					snek.Mooveby(delta_loc);
					if (eating)
					{
						goal.Respaw(rng, brd, snek);
					}
				}
			}
			++snekSpeedupCounter;
	...
	{
		gameIsStatted = wnd.kbd.KeyIsPressed(VK_RETURN);
	}
}
...
void Game::DrawTitel(int x, int y)
{
